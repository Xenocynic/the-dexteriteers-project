...
IOU_NMS = 0.50
FPS_SMOOTH_N = 20
CSV_PATH = "reach_to_mouth_refs.csv"
    ...
    try:
        ...
        while True:
            ...
            if REF_MODE == "bottle" and top_bottle_center:
                ref_pt = top_bottle_center;
                ref_label = "Bottle center"
            elif REF_MODE == "right_index" and right_index_px:
                ref_pt = right_index_px;
                ref_label = "Right index"
            elif REF_MODE == "left_index" and left_index_px:
                ref_pt = left_index_px;
                ref_label = "Left index"
            else:
            ...
            trajectory.append(ref_pt)
        new_file = not os.path.exists(CSV_PATH)
        df = pd.DataFrame(trajectory)
        df['Ref_Point'] = df[0].apply(lambda v: v if v is not None else (None, None))
        # Split into X and Y columns
        df[['Ref_X', 'Ref_Y']] = pd.DataFrame(df['Ref_Point'].tolist(), index=df.index)
        df[['Ref_X', 'Ref_Y']].to_csv(CSV_PATH, index=False)
        print(f"[Saved] Ref point {ref_pt} with scale {mouth_scale:.3f} to {CSV_PATH}")
